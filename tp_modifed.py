...
points.append(np.matrix([-1, -1, .3]))  # ENU  frame, left, out of the screen and up
...
port = None
for p in ports:
    if "Arduino" in p.description or "Teensy" in p.description or "USB" in p.description:
        port = p.device
        break
if port is None:
    raise Exception("No Arduino/Teensy found! Check USB connection.")
print(f"Connected to: {port}")

teensyPilot = serial.Serial(port=port, baudrate=115200, timeout=0.1)
...
def serial_read():
    global q1, q2, q3, q4
    while teensyPilot.in_waiting >= 20:  # Wait until enough bytes
        try:
            line = teensyPilot.readline().decode('utf-8', errors='replace').strip()
            parts = line.replace(' ', '').split(',')  # Remove weird chars
            if len(parts) == 4:
                q1, q2, q3, q4 = map(float, parts)
        except:
            pass
...
while True:
    ...
    for point in points:
        ...
        # rotated2d[2][0] = -x
        distance = 10
        z = -1 / (distance - float(rotated2d[1][0]))
        ...
        if float(point.reshape((3, 1))[1]) > 0:
